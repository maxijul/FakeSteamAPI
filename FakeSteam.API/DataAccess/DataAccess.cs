...
namespace FakeSteam.API.DataAccess
{
    public class DataAccess : IDataAccess
    {
        ...
        // Constructor Configuracion para acceder a la DB
        ...
        // Metodo para traer desde la DB una lista de las categorias de los productos
        ...
        public List<Product> GetProducts(string category, int count)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                string query = $"SELECT TOP {count} * FROM Products WHERE CategoryId=(SELECT CategoryId FROM ProductCategories WHERE Category=@c) ORDER BY newid();";
                command.CommandText = query;
        ...
        public bool InsertUser(User user)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                command.Parameters.Add("@pwd", System.Data.SqlDbType.NVarChar).Value = Encrypt.GetSHA256(user.Password);
        ...
        public string IsUserPresent(string email, string password)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                connection.Open();
                string query = "SELECT COUNT(*) FROM Users WHERE Email='" + email + "' AND Password='" + Encrypt.GetSHA256(password) + "';";
                command.CommandText = query;
                int count = (int)command.ExecuteScalar();
                ...
                query = "SELECT * FROM Users WHERE Email='" + email + "' AND Password='" + Encrypt.GetSHA256(password) + "';";
                command.CommandText = query;
                ...
                while (reader.Read())
                {
                    ...
                    user.Address = (string)reader["Email"];
                ...
                string key = this.configuration["JsonWebToken:Key"];
                string duration = "60";
        ...
        public int InsertPayment(Payment payment)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                command.Parameters.Add("@sc", System.Data.SqlDbType.Int).Value = payment.ShippingCharges;
